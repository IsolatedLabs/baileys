...
async function getAudioWaveform(buffer, logger) {
    try {
        ...
        logger === null || logger === void 0 ? void 0 : logger.warn({ trace: e === null || e === void 0 ? void 0 : e.stack }, 'failed to compute audio waveform');
...
const checkFfmpegAvailable = async () => {
    ...
    try {
        await (0, child_process_1.execFile)('ffmpeg', ['-version'], { timeout: 3000 });
...
async function convertToOpusBuffer(buffer, logger) {
    if (!buffer || (Buffer.isBuffer(buffer) && buffer.length === 0)) {
        throw new boom_1.Boom('cannot convert empty audio buffer to opus', { statusCode: 400 });
    }
        ...
        return await new Promise((resolve, reject) => {
            ...
            ff.on('close', code => {
                if (code === 0) {
                    ...
                    err.stderr = stderr;
                    logger === null || logger === void 0 ? void 0 : logger.debug({ code, stderr }, 'ffmpeg opus conversion failed');
                    reject(err);
                }
            });
            ff.stdin.end(buffer);
    ...
    try {
        ...
        logger === null || logger === void 0 ? void 0 : logger.warn({ err: firstError }, 'opus conversion failed, retrying once');
        try {
            return await attempt();
        } catch (secondError) {
            logger === null || logger === void 0 ? void 0 : logger.warn({ err: secondError }, 'opus conversion retry also failed');
            throw secondError;
        }
    }
}
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts, isPtt, forceOpus } = {}) => {
    ...
    if (mediaType === 'audio' && (isPtt === true || forceOpus === true)) {
        try {
            ffProcess = (0, child_process_1.spawn)('ffmpeg', [
                '-i', 'pipe:0',
                '-c:a', 'libopus',
                '-b:a', '64k',
                '-ar', '48000',
            ...
            ffProcess.stderr.resume();
            finalStream = ffProcess.stdout;
            opusConverted = true;
            
            ffProcess.on('error', (err) => {
                logger === null || logger === void 0 ? void 0 : logger.warn({ err }, 'ffmpeg process error during streaming');
            });
        } catch (error) {
            logger === null || logger === void 0 ? void 0 : logger.warn({ err: error }, 'failed to start ffmpeg streaming, retrying with buffer');
            try {
                const buffer = await (0, exports.toBuffer)(stream);
                ...
                logger === null || logger === void 0 ? void 0 : logger.warn({ err: convError }, 'opus conversion failed, sending original audio');
