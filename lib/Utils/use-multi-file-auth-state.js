...
const fileLockAccess = new Map();
const FILE_LOCK_TTL = 300000; // 5 min
// periodic cleanup of stale file locks to prevent memory leaks
setInterval(() => {
    const now = Date.now();
    for (const [path, lastAccess] of fileLockAccess) {
        if (now - lastAccess > FILE_LOCK_TTL) {
            fileLocks.delete(path);
            fileLockAccess.delete(path);
        }
    }
}, 60000).unref();
const getFileLock = (path) => {
    ...
    fileLockAccess.set(path, Date.now());
    return mutex;
};
/**
