...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const audioCacheKey = (mediaType === 'audio' && fileSha256)
        ? 'audio-meta:' + Buffer.from(fileSha256).toString('base64')
        : null;
    ...
    const [{ mediaUrl, directPath, handle }] = await Promise.all([
        ...
        (async () => {
            try {
                ...
                if (requiresDurationComputation) {
                    let cachedDuration;
                    if (audioCacheKey) {
                        cachedDuration = audioMetadataCache.get(audioCacheKey + ':duration');
                    }
                    if (cachedDuration !== undefined) {
                        uploadData.seconds = cachedDuration;
                        logger === null || logger === void 0 ? void 0 : logger.debug('using cached audio duration');
                    } else {
                        uploadData.seconds = await (0, messages_media_1.getAudioDuration)(bodyPath);
                        logger === null || logger === void 0 ? void 0 : logger.debug('computed audio duration');
                        if (audioCacheKey && uploadData.seconds !== undefined) {
                            audioMetadataCache.set(audioCacheKey + ':duration', uploadData.seconds);
                            if (audioMetadataCache.size > MAX_AUDIO_CACHE) {
                                const firstKey = audioMetadataCache.keys().next().value;
                                if (firstKey) audioMetadataCache.delete(firstKey);
                            }
                        }
                    }
                }
                if (requiresWaveformProcessing) {
                    let cachedWaveform;
                    if (audioCacheKey) {
                        cachedWaveform = audioMetadataCache.get(audioCacheKey + ':waveform');
                    }
                    if (cachedWaveform !== undefined) {
                        uploadData.waveform = cachedWaveform;
                        logger === null || logger === void 0 ? void 0 : logger.debug('using cached audio waveform');
                    } else {
                        uploadData.waveform = await (0, messages_media_1.getAudioWaveform)(bodyPath, logger);
                        logger === null || logger === void 0 ? void 0 : logger.debug('processed waveform');
                        if (audioCacheKey && uploadData.waveform !== undefined) {
                            audioMetadataCache.set(audioCacheKey + ':waveform', uploadData.waveform);
                            if (audioMetadataCache.size > MAX_AUDIO_CACHE) {
                                const firstKey = audioMetadataCache.keys().next().value;
                                if (firstKey) audioMetadataCache.delete(firstKey);
                            }
        ...
        if (didSaveToTmpPath && bodyPath) {
            try {
                ...
                if (error.code === 'EBUSY') {
                    logger === null || logger === void 0 ? void 0 : logger.warn({ path: bodyPath }, 'temp file busy, retrying');
