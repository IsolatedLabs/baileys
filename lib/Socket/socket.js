...
const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
        ...
        // enforce rate limit backoff if we've been rate limited
        const now = Date.now();
        const timeSinceRateLimit = now - lastRateLimitTime;
        if (lastRateLimitTime > 0 && timeSinceRateLimit < 1000) {
            const waitMs = Math.min(30000, config.backoffDelayMs || 5000);
            if (timeSinceRateLimit < waitMs) {
                await (0, Utils_1.delay)(waitMs - timeSinceRateLimit);
            }
        }
